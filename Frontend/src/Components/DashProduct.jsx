...
export default function DashProduct() {
    ...
    const fetchProducts = async () => {
      try {
        const res = await fetch(
          `/api/products/getproducts?searchTerm=${searchTerm}`
        );
  ...
  const generatePDFReport = () => {
    ...
    html2pdf()
      .from(content)
      .set({ margin: 1, filename: "product_report.pdf" })
      .save();
  };
  ...
  const handleAssignFeature = async (productId) => {
    try {
      const res = await fetch(
        `/api/products/featureproduct/${productId}/${currentUser._id}`,
        {
          method: "PUT",
          headers: {
  ...
  const handleReassignFeature = async (productId) => {
    try {
      const res = await fetch(
        `/api/products/unfeatureproduct/${productId}/${currentUser._id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  return (
    <div className="table-auto overflow-x-scroll md:mx-auto p-3 scrollbar scrollbar-track-slate-100 scrollbar-thumb-slate-300 dark:scrollbar-track-slate-700 dark:scrollbar-thumb-slate-500">
      <div className="flex justify-between pb-4">
        ...
        <Link className="flex justify-end" to="/Inventorysummary">
