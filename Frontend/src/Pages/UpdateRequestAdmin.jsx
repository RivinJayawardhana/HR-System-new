...
export default function UpdateRequestAdmin() {
    ...
    useEffect(() => {
      ...
      const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const res = await fetch(`/api/request/update_status/${formData._id}`, {
                method: 'PUT',
                headers: {
        ...
        <form className="flex max-w-3xl flex-col mx-auto pb-10" onSubmit={handleSubmit} >
            <div className='flex flex-col justify-center'>
                <div>
                    ...
                    <TextInput type='date' required id='email' className='p-2 mb-2' onChange={(e) => setFormData({ ...formData, date: e.target.value })} value={formData.date} placeholder="Date"/>
...
{/*                     
            ...
            <button type='submit' className="bg-indigo-500 hover:bg-indigo-600 p-3 rounded-lg text-white " >Update Request</button>
            {publishError && (
